...
class GoogleScrapeTool:
   @tool("Scrape website content to extract information about courses")
   def google_scrape(course_query: str ):
      """Useful for scrapping and summarizing content from a website by
         searching google.com for specific queries like 'machine learning 
         in python course' 'course' must be in the query"""
      try:
            SBR_WEBDRIVER = 'https://brd-customer-hl_7b582a0b-zone-scraping_browser1:82ci69ne32wu@brd.superproxy.io:9515' 
            ...
            # Find all elements that could be course links
            courses_elements = driver.find_elements(By.CLASS_NAME, 'sVXRqc')
            courses_links = [elem.get_attribute('href') for elem in courses_elements if 'course' in elem.text] 
            driver.quit()
            main_list = []
...
if __name__ == "__main__":
   ...
   result = GoogleScrapeTool.google_scrape(course_query)
